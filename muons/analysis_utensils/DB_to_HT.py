...
def detection_with_pure_Hough(
    ...
    ret, inliers = perform_circleModel(ret, full_clusters_fps)
    ret = perform_hough(ret, full_clusters_fps)
    to_continue, ret = check_photon_count(
        full_cluster_mask, ret, number_of_photons)
    if to_continue == False:
        return ret
    to_continue = check_ring_radius(ret)
    if to_continue == False:
        return ret
    # print("find inliers_count")
    # inliers = find_inliers_count(epsilon, full_clusters_fps, ret)
    to_continue, ret, field_of_view_radius = (
        check_ring_overlap_with_FOV(ret, event))
    if to_continue == False:
        return ret
    to_continue, ret = check_arrival_time(full_clusters_fps, ret)
    if to_continue == False:
        return ret
    ...
    if to_continue == False:
        return ret
    to_continue, ret = check_ratio_inside_circle(onoff, ret, number_of_photons)
    if to_continue == False:
        return ret
    ...
    if to_continue == False:
        return ret
    to_continue, ret = find_photon_ratio(inliers, number_of_photons, ret)
    if to_continue == False:
        return ret
